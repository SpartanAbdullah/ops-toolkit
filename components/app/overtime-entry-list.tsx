...
export function OvertimeEntryList({
  ...
  // Track entries currently being approved so we can paint them optimistically
  const [approvingIds, setApprovingIds] = useState<Set<string>>(new Set());
  const [, startTransition] = useTransition();

  const handleApprove = (row: OvertimeLedgerRow) => {
...
function EntryCard({
  ...
  return (
    <div
      className={cn(
        "rounded-2xl border border-border bg-white p-4 shadow-card transition",
        isApproving && "border-mint-100 bg-mint-50/40",
      )}
    >
      ...
      <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-2xs text-text-muted">
        <span className="inline-flex items-center gap-1">
          <Calendar className="h-3 w-3" /> {row.totalWorkedLabel} worked
        </span>
      ...
      {(row.notes || row.approvalComment) && (
        <div className="mt-3 rounded-lg bg-surface-muted px-3 py-2 text-sm leading-5 text-text-secondary">
          ...
          {row.approvalComment ? <p className="mt-1 italic">&ldquo;{row.approvalComment}&rdquo;</p> : null}
        </div>
      )}

      {showAdminActions && row.status === "pending" && !isApproving ? (
